...
class FeatureSynthesis:
    ...
    def synthesizeFeatures(self,intFeat, boolFeat, baseFeatureVectors):
        ...
        for postFeaturesIdxs  in [ (intFeatures[newIdx], newIdx) for newIdx in range(len(intFeatures)) if intFeatures[newIdx].isNew != None and intFeatures[newIdx].isNew]:
            ...
            stringExpr = str(sygusExpr[0]).replace("\n", " ")
            msg2 = f"number of features synthesize: {len(sygusExpr)}"
            print(msg2)
            logger1.info(msg2)
            msg3= f"======================================== synthesized feature: {stringExpr}"
            print(msg3)
            logger1.info(msg3)
            
            synthesizedFeatures += (PrecisFeature(True, stringExpr, str(sygusExpr[0].sort()), None, sygusExpr[0]),)#assumes only return list of length == 1(sygusExpr) 
            #print(sygusExpr.ctx)
