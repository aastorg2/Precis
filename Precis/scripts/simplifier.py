...
def convert(postcondition):
    ...
    disjunctiveSplit = [ term.strip() for term in disjunctiveSplit]
    # remove surround parens from terms if present
    for term_idx in range(len(disjunctiveSplit)):
        if disjunctiveSplit[term_idx][0] == '(' and disjunctiveSplit[term_idx][-1] == ')':
            disjunctiveSplit[term_idx] = disjunctiveSplit[term_idx][1:-1]
            
    #disjunctiveSplit[0] = disjunctiveSplit[0].replace("(", "")
    #predicates is a list of list of string predicates
    predicates = [predicate.split(" && ") for predicate in disjunctiveSplit]
